...
@section('content')
    ...
    <form action="{{ route('peliculas.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <textarea name="sinopsis" id="sinopsis" class="form-control" rows="3" required>{{ old('sinopsis') }}</textarea>
        </div>

        <div class="mb-3">
            <label for="trailer_url" class="form-label">URL del tráiler</label>
            <input type="url" name="trailer_url" id="trailer_url" class="form-control" value="{{ old('trailer_url') }}" required>
        </div>

        <div class="mb-3">
            <label for="calidad" class="form-label">Calidad</label>
            <input type="text" name="calidad" id="calidad" class="form-control" value="{{ old('calidad') }}" required>
        </div>
